	...
	def update(self):
		...
		if self.vida > 0:					
			if teclas[K_LEFT] and self.rect.x>=10:
				self.rect.x -= 10
			elif teclas[K_RIGHT] and self.rect.x<=990-self.rect.width:
				self.rect.x += 10
			if teclas[K_UP] and self.rect.y>=10:
				...
				if self.rect.y==0 and self.estado == "subiendo":
					...
					self.estado = "bajando"
			elif teclas[K_DOWN] and self.rect.y<=440-self.rect.height:
				self.rect.y += 10
				self.image = self.imagenes[0]
				if self.rect.y==380 and self.estado == "bajando":
					self.puntos = self.puntos + 1
